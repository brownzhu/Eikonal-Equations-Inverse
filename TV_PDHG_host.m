...
% This program applies the PDHD algorithm to solve the
...
function [u, w1,w2, Energy, Dgap, TimeCost, itr] = TV_PDHD(w1,w2,f,lbd,NIT,GapTol);
...
Dual = (lbd/2.)*sum(sum(f.^2 - (1.0/lbd^2)*(DivW-g).^2)); 
...
Primal = sum(sum(gu_norm + (lbd/2)*(u-f).^2));
...
for itr=1:NIT
  ...
  Dual = (lbd/2.)*(sum(sum(f.^2)) - (1.0/lbd^2)*sum(sum((DivW-g).^2))); 
  ...
  %Dgap(itr+1)= (Primal(itr+1)-Dual(itr+1)) / (abs(Primal(itr+1)) + abs(Dual(itr+1)));
  Dgap= (Primal-Dual) / (abs(Primal) + abs(Dual));
