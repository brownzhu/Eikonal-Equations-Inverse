...
duality_r = 1.2;         % duality mapping J_r r value.
mu_0 = 0.8*(1 - 1/1.01); 
%mu_0 = 1.8*(1 - eta - (1+ eta)/1.5);  % step size parameter
...
% Nx_src = 6;  % bottom & top edges
% Nz_src = 3;  % left & right edges
% 
% d_ratio = 0.18;   % 距离边界的比例
% 
% d_phys_x = d_ratio * (x_max - x_min);
% d_phys_z = d_ratio * (z_max - z_min);
% 
% kx = max(2, round(d_phys_x / dx));
% kz = max(2, round(d_phys_z / dy));
% 
% fixed_pt_list = [];
% 
% % ---- bottom side (inside), 去掉左右角 ----
% j_list = round(linspace(1+kx, J-kx, Nx_src+2));
% j_list = j_list(2:end-1);  % 去掉首尾
% i_list = kz * ones(size(j_list));
% fixed_pt_list = [fixed_pt_list;
%     zeros(numel(j_list),1), j_list(:), i_list(:)];
% 
% % ---- top side (inside), 去掉左右角 ----
% i_list = (I-kz+1) * ones(size(j_list));
% fixed_pt_list = [fixed_pt_list;
%     zeros(numel(j_list),1), j_list(:), i_list(:)];
% 
% % ---- left side (inside), 去掉上下角 ----
% i_list = round(linspace(1+kz, I-kz, Nz_src+2));
% i_list = i_list(2:end-1);   % 去掉首尾
% j_list = kx * ones(size(i_list));
% fixed_pt_list = [fixed_pt_list;
%     zeros(numel(i_list),1), j_list(:), i_list(:)];
% 
% % ---- right side (inside), 去掉上下角 ----
% j_list = (J-kx+1) * ones(size(i_list));
% fixed_pt_list = [fixed_pt_list;
%     zeros(numel(i_list),1), j_list(:), i_list(:)];
% figure; hold on;
% 
% % 将索引转换为物理坐标
% src_x = x(fixed_pt_list(:,2));   % j -> x
% src_z = z(fixed_pt_list(:,3));   % i -> z
% 
% plot(src_x, src_z, 'ro', 'MarkerFaceColor','r');
% 
% % 画出矩形边界
% plot([x_min x_max x_max x_min x_min], ...
%      [z_min z_min z_max z_max z_min], 'k-', 'LineWidth', 1.2);
% 
% axis equal;
% xlabel('x'); ylabel('z');
% title('Source locations in physical coordinates');
% box on;
% drawnow;
...
p_imp        = 0.05;   % 边界点中脉冲噪声比例
A_rel        = 0.1;    % 脉冲幅度相对边界 range
sigma = 0.2; alpha = 0.05;      % Gaussian 噪声幅度相对边界 range
...
for p_num = 1:num_sources
    ...
    %% 脉冲噪声
    A   = A_rel * range_Tb;
    sgn = sign(randn(numel(imp_idx),1));   % ±1
    T1  = T0;
    T1(imp_idx) = T1(imp_idx) + A .* sgn;

    %% Gaussian 噪声
    noise_g = sigma*randn(numel(gauss_idx),1);
   T1(gauss_idx) = T1(gauss_idx) .* (1 + alpha * noise_g);
%     Tstar_noisy(:,:,p_num) = Tstar_all(:,:,p_num) .*(1+ alpha*noise);
    %% 保存
    Tstar_noisy(:,:,p_num) = T1;
end
err_level=0;
 for p_num = 1:size(fixed_pt_list, 1)
...
err_level = err_level^(1/duality_r); disp(err_level);

%% ========== add Gaussian noise on boundary ==========
% Tstar_noisy = Tstar_all;
% 
% bmask = false(I,J);
% bmask(1,:) = true;
...
% for p_num = 1:num_sources
%     noise = zeros(I,J);
%     noise(bmask) = sigma * randn(nnz(bmask),1);
%     Tstar_noisy(:,:,p_num) = Tstar_all(:,:,p_num) .*(1+ alpha*noise);
% end
...
xlabel('x', 'FontSize', 12)
ylabel('z', 'FontSize', 12)
title(sprintf('c_{appro} (%s, \\beta=%.2f)', regularization_type, beta), 'FontWeight', 'bold', 'FontSize', 12)
caxis([cmin_sol, cmax_sol])
colorbar
axis xy equal tight
...
fig3 = figure('Position', [100, 100, 1500, 450]);  % 宽一点
sgtitle(sprintf('Convergence - Rectangular Domain (%s, \\beta=%.2f)', ...
    regularization_type, beta), 'FontSize', 14, 'FontWeight', 'bold');
...
% =======================
subplot(1, 2, 1)
semilogy(alpha_set, '-o', ...
    'LineWidth', 1.5, ...
    'MarkerIndices', 1:5:numel(alpha_set), ...
    'MarkerSize', 4, ...
    'Color', [0.4660, 0.6740, 0.1880])

grid on
title('Step Size ', 'FontWeight', 'bold', 'FontSize', 12)
xlabel('Iteration', 'FontSize', 11)
...
%% ========== Figure 4: Traveltime Field Example ==========
fig4 = figure('Position', [100, 100, 1000, 400]);
...
%% ========== Figure 5: Observation Data (Clean vs Noisy) + Boundary 1D ==========
fig5 = figure('Position', [100, 100, 1600, 850]);
...
% imagesc(x, z, Tnoisy);
% axis xy equal tight;
% title('Noisy Observation: $\tilde{T}^*$ (Noise on $\partial\Omega$)', ...
...
fig6 = figure('Position', [100, 100, 1600, 850]);
sgtitle(sprintf('Observation Data Comparison (Clean vs Impulse-Noisy) | Source #%d', 1), ...
        'FontSize', 14, 'FontWeight', 'bold');
% -------- 主图 subplot --------
ax_main = subplot(1,3,1);
h1 = plot(s, bc, 'LineWidth', 1.8); hold on
h2 = plot(s, bn, '--', 'LineWidth', 1.4);
...
legend([h1 h2], {'$T^*$ (clean)', '$\tilde{T}^*$ (noisy)'}, ...
       'Location','best','Interpreter','latex');

title('Boundary Curves (Clean vs Noisy)');
xlabel('s'); ylabel('T'); grid on;

% -------- 自动随机选择一段 --------
L = round(0.15 * numel(s));  % 放大长度 15%
start_idx = randi([1, numel(s)-L]);
end_idx = start_idx + L - 1;

x_zoom = [s(start_idx) s(end_idx)];
y_zoom = [min(min(bc(start_idx:end_idx)), min(bn(start_idx:end_idx))) ...
          max(max(bc(start_idx:end_idx)), max(bn(start_idx:end_idx)))];

% -------- 小坐标轴 inset 放在主图内，不压曲线 --------
main_pos = get(ax_main, 'Position');  
inset_width  = 0.22 * main_pos(3);  
inset_height = 0.22 * main_pos(4);  
inset_left   = main_pos(1) + 0.65*main_pos(3);  
inset_bottom = main_pos(2) + 0.65*main_pos(4);  

ax_inset = axes('Position', [inset_left, inset_bottom, inset_width, inset_height]);
box on; hold on
plot(s, bc, 'LineWidth', 1.2)
plot(s, bn, '--', 'LineWidth', 1.2)
xlim(x_zoom)
ylim(y_zoom)
set(ax_inset,'FontSize',10)
title('Random segment','FontSize',10)

% -------- 在主图上画黑色矩形标出放大区域，线宽细一些 --------
rectangle('Position',[x_zoom(1), y_zoom(1), diff(x_zoom), diff(y_zoom)], ...
          'EdgeColor','k','LineWidth',0.8,'LineStyle','-');  % 边框细
...
title('Boundary Noise (noisy) | \Delta on \partial\Omega', 'FontWeight','bold');
