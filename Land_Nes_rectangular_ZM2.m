...
duality_r = 1.8;         % duality mapping J_r r value.
mu_0 = 0.8*(1 - 1/1.01); 
...
p_imp        = 0.04;   % 边界点中脉冲噪声比例
A_rel        = 0.2;    % 脉冲幅度相对边界 range
noise_level  = 0.01;   % Gaussian 噪声幅度相对边界 range
...
for p_num = 1:num_sources
    ...
    %% Gaussian 噪声
    gaussian_noise = noise_level * range_Tb * randn(numel(gauss_idx),1);
    T1(gauss_idx)  = T1(gauss_idx) + gaussian_noise;
...
resn_Lr = 100; k=1;
while resn_Lr>=err_level& k<=kmax
    energy_p = 0;
    cstar = 0;
    resn_Lr = 0;
    if Nesterov
