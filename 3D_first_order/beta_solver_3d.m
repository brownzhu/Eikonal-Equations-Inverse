function beta = beta_solver_3d(T, T_star, dx, dy, dz)
% BETA_SOLVER_3D
...
for i = 2:I-1
    for j = 2:J-1
        for k = 2:K-1
            ay_p = -(T(i+1,j,k)-T(i,j,k))/dy;
            ay_m = -(T(i,j,k)-T(i-1,j,k))/dy;
            ax_p = -(T(i,j+1,k)-T(i,j,k))/dx;
            ax_m = -(T(i,j,k)-T(i,j-1,k))/dx;
    ...
    for i = ord(1):ord(2):ord(3)
        for j = ord(4):ord(5):ord(6)
            for k = ord(7):ord(8):ord(9)
                ...
                denom = (a_p(i,j,k)-a_m(i,j,k))/dy ...
                      + (b_p(i,j,k)-b_m(i,j,k))/dx ...
                ...
                num = (a_p(i,j,k)*beta(i-1,j,k) - a_m(i,j,k)*beta(i+1,j,k))/dy ...
                    + (b_p(i,j,k)*beta(i,j-1,k) - b_m(i,j,k)*beta(i,j+1,k))/dx ...
